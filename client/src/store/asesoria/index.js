...
const actions = {
    ...
    async crearAsesoria({commit}, credenciales) {
		commit('CREAR')
		const {grosor,nombre, descripcion , estado } = credenciales
		await this.$apollo.defaultClient.mutate({
			mutation: CREAR_ASESORIA_MUTATION,
			variables: {grosor,nombre, descripcion , estado }
		}).then(response => {
    ...
    async editarAsesoria({commit}, credenciales) {
		commit('EDITAR')
		const {id_asesoria, grosor,nombre, descripcion , estado } = credenciales
		await this.$apollo.defaultClient.mutate({
			mutation: EDITAR_ASESORIA_MUTATION,
			variables: {id_asesoria, grosor,nombre, descripcion , estado }
		}).then(response => {
