...
export default Vue.component('Asesoria', {
    $validates: 1,
    components:{
	...
	computed: {
		...mapGetters({ 
            ...
            registro_creado: "Asesoria/getCreado",
            registro_editado: "Asesoria/getEditado",
            registro_eliminado: "Asesoria/getEliminado",
            error: "Asesoria/error" , 
        })
	},
	methods: {
        async iniciar () {
            this.$q.loading.show()
			await this.$store.dispatch("Asesoria/cargarAsesoria").then(res => {
				this.$q.loading.hide()
				if(this.error){
        ...
        async guardar_editar(){
            ...
            await this.$store.dispatch("Asesoria/editarAsesoria", { codigo : this.editar_codigo, titulo:editar_titulo, descripcion:editar_descripcion , estado:est }).then(res => {
                console.log(res)
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
        ...
        async guardar_nuevo() {
            ...
            if(nuevo_titulo == ''){
                ...
                this.$q.loading.show()
                await this.$store.dispatch("Asesoria/crearAsesoria", { titulo: nuevo_titulo, descripcion:nuevo_descripcion , estado:est }).then(res => {
                    this.$q.loading.hide()
                    if(this.error){
                        var message = this.error.message.replace('GraphQL error: ','')
                        this.$q.notify({
        ...
        async guardar_eliminar(){
            ...
            await this.$store.dispatch("Asesoria/eliminarAsesoria", { id:this.parametros_tabla.selected}).then(res => {
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
                    this.$q.notify({
