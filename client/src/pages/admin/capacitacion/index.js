...
export default Vue.component('Capacitacion', {
    $validates: 1,
    components:{
	...
	computed: {
		...mapGetters({ 
            ...
            registro_creado: "Capacitacion/getCreado",
            registro_editado: "Capacitacion/getEditado",
            registro_eliminado: "Capacitacion/getEliminado",
            error: "Capacitacion/error" , 
        })
	},
	methods: {
        async iniciar () {
            this.$q.loading.show()
			await this.$store.dispatch("Capacitacion/cargarCapacitacion").then(res => {
				this.$q.loading.hide()
				if(this.error){
        ...
        async guardar_editar(){
			this.$q.loading.show()
            const {editar_titulo, editar_descripcion , editar_link,  editar_banner, editar_estado} = this
            var est = editar_estado.value
            await this.$store.dispatch("Capacitacion/editarCapacitacion", { codigo : this.editar_codigo, titulo:editar_titulo, descripcion:editar_descripcion ,link: editar_link, banner: editar_banner, estado:est }).then(res => {
                console.log(res)
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
        ...
        async guardar_nuevo() {
            ...
            await this.$store.dispatch("Capacitacion/crearCapacitacion", { titulo: nuevo_titulo, descripcion:nuevo_descripcion , link: nuevo_link, banner: nuevo_banner, estado:est }).then(res => {
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
                    this.$q.notify({
        ...
        async guardar_eliminar(){
            this.$q.loading.show()
            await this.$store.dispatch("Capacitacion/eliminarCapacitacion", { id:this.parametros_tabla.selected}).then(res => {
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
                    this.$q.notify({
