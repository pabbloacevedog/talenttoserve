...
export default Vue.component('Proveedor', {
	...
	methods: {
        ...
        async guardar_editar(){
            ...
            await this.$store.dispatch("Proveedor/editarProveedor", { codigo : this.editar_codigo,proveedor:editar_proveedor,direccion:editar_direccion,telefono:editar_telefono,email:editar_email,web:editar_web,categoria:editar_categoria, descripcion:editar_descripcion ,banner: editar_banner, estado:est }).then(res => {
                console.log(res)
                this.$q.loading.hide()
                if(this.error){
                    var message = this.error.message.replace('GraphQL error: ','')
